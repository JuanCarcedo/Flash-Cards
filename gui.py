...
class UsernameManager(ctk.CTk):
    ...
    def __basic_structure(self):
        ctk.CTkLabel(self, text='Please, enter your username:',
                      font=('Arial', 15)).pack()
        self.input_username = tk.StringVar()
        entry_username = ctk.CTkEntry(self,
                                      textvariable=self.input_username)
...
class App(ctk.CTk):
    ...
    def __init__(self, username: str = ''):
        ...
        # Get the data
        self.data = DataManager(self._username)
    ...
    # Building the GUI =================================================================
    def __build_welcome(self) -> None:
        self.legend_label = ctk.CTkLabel(self,
                                         text=f'Player: {self._username}',
                                         font=ctk.CTkFont(size=25, weight='bold'))
        self.legend_label.grid(row=0, column=0, columnspan=3,
                               padx=10, pady=(10, 10))

    def __build_words(self) -> None:
        """Flashcard building. """
        self.__label_learning_word = ctk.CTkLabel(self,
                                                  text=self.data.get_word_to_guess(),
                                                  font=ctk.CTkFont(size=20))
        self.__label_learning_word.grid(row=1, column=0, columnspan=3,
                                        padx=10, pady=(15, 15))

    def __build_option_menus(self) -> None:
        ...
        self.frame_options.grid(row=0, column=0, rowspan=3, columnspan=1,
                                padx=(15, 5))
        
        self.btn_new_word = ctk.CTkButton(self.frame_options,
                                          command=self.new_word,
                                          text='New Word')
        self.btn_new_word.grid(row=0, column=0, padx=10, pady=10)

        self.btn_exit_save = ctk.CTkButton(self.frame_options,
                                           command=self.exit_program,
                                           text='Save & Exit',
                                           fg_color='transparent',
                                           border_width=1)
        self.btn_exit_save.grid(row=2, column=0,
                                padx=10, pady=(40, 10))

        # Check answers
        self.frame_selection = ctk.CTkFrame(self.frame_user, width=100)
        self.frame_selection.grid(row=0, column=1, rowspan=3, columnspan=1,
                                  padx=(15, 5),
                                  pady=10)
        ...
        self.radio_btn_dont_know = ctk.CTkRadioButton(master=self.frame_selection,
                                                      ...
                                                      value=0,
                                                      text='I don\'t know')
        self.radio_btn_dont_know.grid(row=0, column=0,
                                      padx=10, pady=10,
                                      sticky='e')
        ...
        self.radio_btn_i_know = ctk.CTkRadioButton(master=self.frame_selection,
                                                   ...
                                                   value=1,
                                                   text='I know! :)')
        self.radio_btn_i_know.grid(row=1, column=0,
                                   padx=10, pady=10,
                                   sticky='e')

        self.btn_check_answer = ctk.CTkButton(self.frame_selection,
                                              command=self.check_answer,
                                              text='Check answer')
        self.btn_check_answer.grid(row=2, column=0,
                                   padx=(10, 10), pady=(5, 10))

    def __build_bottom(self) -> None:
        """Create the bottom layer of the app"""
        self.label_bottom = ctk.CTkLabel(self, text='Juan Carcedo © 2022-2023',
                                         font=ctk.CTkFont(size=10))
        self.label_bottom.grid(row=3, column=0, columnspan=3,
                               padx=10, pady=(10, 10))

    # Logics / buttons =================================================================
    def check_answer(self) -> None:
        """Show the correct answer. . """
        if self.get_radio_button_status() == 1:
            # User knows the word -> Save the word as user knows it
    ...
    def wait_time(self, time_to_wait: int = M_SECONDS_WAIT) -> None:
    ...
    def exit_program(self) -> None:
        """ Save and Exit """
        self.data.save_data(self._username)
        self.destroy()
    ...
    @username.setter
    def username(self, new_user: str = '') -> None:
        """ Set current username.
        :param new_user: str: Current User.
        """
